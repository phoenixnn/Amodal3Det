...
void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int count = bottom[0]->count(); // all elements in bottom[0]
  caffe_gpu_sub(
      count,
      bottom[0]->gpu_data(),        // copy data to gpu
      bottom[1]->gpu_data(),
  ...
  SmoothL1Forward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, diff_.gpu_data(), errors_.mutable_gpu_data()); // compute smooth L1
  CUDA_POST_KERNEL_CHECK;

  Dtype loss;
  caffe_gpu_asum(count, errors_.gpu_data(), &loss);
  top[0]->mutable_cpu_data()[0] = loss / bottom[0]->num(); // average loss
}
